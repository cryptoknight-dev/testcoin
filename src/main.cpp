...
#if defined(NDEBUG)
#error "Pivxcoin cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight < Params().LAST_POW_BLOCK())
        ...
        nSubsidy = 40.5 * COIN;
    else if (nHeight > 14401 && nHeight <= 36000)
        nSubsidy = 36 * COIN;
    else if (nHeight > 36001 && nHeight <= 51120)
        nSubsidy = 31.5 * COIN;
    else if (nHeight > 51121 && nHeight <= 79200)
        nSubsidy = 27 * COIN;
    else if (nHeight > 79201 && nHeight <= 136800)
        nSubsidy = 22.5 * COIN;
    else if (nHeight > 136801 && nHeight <= 201600)
        nSubsidy = 18 * COIN;
    else if (nHeight > 201601 && nHeight <= 273600)
        nSubsidy = 13.5 * COIN;
    else if (nHeight > 273601 && nHeight <= 360000)
        nSubsidy = 9 * COIN;
    else
        nSubsidy = 4.5 * COIN;
...
{
    RenameThread("pivxcoin-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // PIV: We use certain sporks during IBD, so check to see if they are
